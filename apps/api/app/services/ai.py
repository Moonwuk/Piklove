...
class OpenAIProvider:
    ...
    async def analyze_conversation(self, context):
        return await self._structured(
            self.s.openai_analysis_model,
            Path(__file__).parents[1].joinpath("prompts/conversation_analyzer.md").read_text(),
            context,
            ConversationAnalysis,
        )

    async def generate_replies(self, context, analysis):
        combined = context.model_copy()
        prompt = (
            Path(__file__).parents[1].joinpath("prompts/reply_generator.md").read_text()
            + "\nAnalysis:\n"
...
class AIContextBuilder:
    async def build(self, db, user_id, conversation) -> AIConversationContext:
        ...
        ctx = [
            ContextMessage(
                direction=m.direction.value, text=m.text, telegram_created_at=m.telegram_created_at
            )
            for m in messages
        ]
...
class SuggestionService:
    def __init__(self, provider: LLMProvider, builder: AIContextBuilder | None = None):
        self.provider = provider
        self.builder = builder or AIContextBuilder()
    ...
    async def generate(self, db, user_id, conversation):
        ...
        db.add_all(
            [g, UsageEvent(user_id=user_id, type="ai_generation", quantity=1, event_metadata={})]
        )
        await db.commit()
        return g
