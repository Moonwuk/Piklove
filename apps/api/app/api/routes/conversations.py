...
from app.api.deps import current_user_id, get_suggestion_service, get_telegram_gateway
...
from app.integrations.telegram.client import TelegramGateway
...
async def mode(
    conversation_id: str, body: ModeBody, user_id=Depends(current_user_id), db=Depends(get_db)
):
    c = await access.conversation(db, user_id, conversation_id)
    c.ai_mode = body.mode
    await db.commit()
...
async def suggestions(
    ...
    try:
        g = await service.generate(db, user_id, c)
...
async def _send(
    ...
    if g.expires_at.replace(tzinfo=UTC) <= now:
    ...
    await db.commit()
    try:
        result = await telegram.send_business_message(
    ...
    db.add(
        Message(
            ...
            telegram_created_at=datetime.fromtimestamp(
                result.get("date", int(now.timestamp())), UTC
            ),
        )
    )
    await db.commit()
...
async def send(
    ...
    user_id=Depends(current_user_id),
    db=Depends(get_db),
    telegram: TelegramGateway = Depends(get_telegram_gateway),
):
    ...
    return await _send(conversation_id, g.id, option.text, user_id, db, idempotency_key, telegram)
