...
async def security(request:Request,call_next):
 request.state.request_id=request.headers.get("X-Request-ID",str(uuid.uuid4())); response=await call_next(request); response.headers.update({"X-Request-ID":request.state.request_id,"X-Content-Type-Options":"nosniff","Referrer-Policy":"no-referrer","Content-Security-Policy":"default-src 'none'; frame-ancestors 'none'"}); return response
@app.exception_handler(Exception)
async def errors(request,exc):
 from fastapi import HTTPException
 if isinstance(exc,HTTPException): return JSONResponse({"error":{"code":str(exc.detail),"message":str(exc.detail).replace("_"," ").title(),"request_id":request.state.request_id}},status_code=exc.status_code)
 return JSONResponse({"error":{"code":"INTERNAL_ERROR","message":"Internal server error","request_id":request.state.request_id}},status_code=500)
