...
def build(now=1000, token="token"):
    d = {
        "auth_date": str(now),
        "query_id": "q",
        "user": json.dumps({"id": 123, "first_name": "A"}, separators=(",", ":")),
    }
...
def test_valid():
    assert validate_init_data(build(), "token", 300, 1000)["id"] == 123


def test_invalid_hash():
    with pytest.raises(InitDataError, match="invalid hash"):
        validate_init_data(build() + "x", "token", 300, 1000)


def test_expired():
    with pytest.raises(InitDataError, match="expired"):
        validate_init_data(build(), "token", 300, 2000)
